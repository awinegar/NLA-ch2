...
#library(betapart) #Baselga package, used in V2 analyses, not needed for V3 analyses.  
...
##################################################################################################
####RADIOMETRIC DATING LOGISTIC REGRESSION                                                       #
##################################################################################################
...
#Note a real logistic looking curve
logr.plot<- logr.plot + labs(x="Length of core (cm)", y="Designated Pre-1850") 
logr.plot<- logr.plot + theme_bw()
logr.plot<- logr.plot + theme(axis.text.x = element_text(colour="black", size=16))
logr.plot<- logr.plot + theme(axis.text.y = element_text(colour="black", size=16))
logr.plot<- logr.plot + theme(axis.title.x = element_text(size = rel(2), angle=00))
logr.plot<- logr.plot + theme(axis.title.y = element_text(size = rel(2), angle=90))
...
ctbl2<- table(dates.dat$Pre_1850_binary_Brothers, dates.dat$Pre_1850_binary)
...
##Make figure 3 for manuscript, map of historical/2007 total BD and LCBD
...
surf.abund.bdc$part
#1st value = Total beta div  ##WHY IS THIS DIFFERENT FROM USING beta.div???????
...
#Total BD (from  beta.div.comp())
bdc.total.plot<- ggplot(all.bdc, aes(x=Sediment_type, y=Total, colour=Ecoregion)) + geom_point(size=4) 
bdc.total.plot<- bdc.total.plot + annotate("segment", x=1, xend=2, y=0.435, yend=0.429) #All (not best way of making this visual but will do for now)
bdc.total.plot<- bdc.total.plot + annotate("segment", x=1, xend=2, y=0.461, yend=0.428) #CPL
bdc.total.plot<- bdc.total.plot + annotate("segment", x=1, xend=2, y=0.388, yend=0.388) #NAP
bdc.total.plot<- bdc.total.plot + annotate("segment", x=1, xend=2, y=0.354, yend=0.388) #SPL
bdc.total.plot<- bdc.total.plot + annotate("segment", x=1, xend=2, y=0.396, yend=0.403) #TPL
bdc.total.plot<- bdc.total.plot + annotate("segment", x=1, xend=2, y=0.438, yend=0.409) #UMW
bdc.total.plot<- bdc.total.plot + annotate("segment", x=1, xend=2, y=0.438, yend=0.428) #WMT
bdc.total.plot<- bdc.total.plot + ggtitle("Total BD (Spatial)")

#Proportion explained by Replacement component 
bdc.repl.plot<- ggplot(all.bdc, aes(x=Sediment_type, y=Repl_Total, colour=Ecoregion)) + geom_point(size=4) 
#bdc.repl.plot<- bdc.repl.plot + annotate("segment", x=1, xend=2, y=0.935, yend=0.97) #All (not best way of making this visual but will do for now)
#bdc.repl.plot<- bdc.repl.plot + annotate("segment", x=1, xend=2, y=1, yend=0.99) #CPL
#bdc.repl.plot<- bdc.repl.plot + annotate("segment", x=1, xend=2, y=1, yend=0.99) #NAP
#bdc.repl.plot<- bdc.repl.plot + annotate("segment", x=1, xend=2, y=0.949, yend=1) #SPL
#bdc.repl.plot<- bdc.repl.plot + annotate("segment", x=1, xend=2, y=1, yend=0.94) #TPL
#bdc.repl.plot<- bdc.repl.plot + annotate("segment", x=1, xend=2, y=0.872, yend=0.964) #UMW
#bdc.repl.plot<- bdc.repl.plot + annotate("segment", x=1, xend=2, y=0.942, yend=0.944) #WMT
bdc.repl.plot<- bdc.repl.plot + ggtitle("Proportion of spatial BD explained by Replacement")

#Proportion explained by AbDiff component
bdc.abdiff.plot<- ggplot(all.bdc, aes(x=Sediment_type, y=AbDiff_Total, colour=Ecoregion)) + geom_point(size=4) 
bdc.abdiff.plot<- bdc.abdiff.plot + ggtitle("Proportion of spatial BD explained by AbDiff")

bdc.combo<- grid.arrange(bdc.repl.plot, bdc.abdiff.plot, nrow=1)
...
##Data to use (must have same sites and rows) --> species lists must be the same. 
##Input matrix 1 (historical data)
#nla.diat.new.hist[,3:1247]  ##NOT SURE WHERE THESE ARE COMING FROM- GO BACK TO VERSION 2 

##Input matrix 2 (surface data)
#nla.diat.new.surf[,3:1247]

##AT THIS POINT: do not have matrices of QUANTITATIVE data with same species
#the quantitative matrices for historical and surface data have different species
#numbers, so need to create these. 
...
##Started editing here May 21, 2015
...
#CONTINUE HERE- PLOT THESE RESULTS FOR B, C, D
...
#Visulizations with B, C, D. 

#Basic lm()s. Work on ANCOVA and MEMs. 
b.lat<- lm(B~LAT_DD, data=temp.D2quant.mat3) #adj. R2 = -0.00498, p=0.7733
b.long<- lm(B~LONG_DD, data=temp.D2quant.mat3) #adj. R2 = 0.08191, p=<0.05

c.lat<- lm(C~LAT_DD, data=temp.D2quant.mat3) #adj. R2 = -0.001345, p=0.3871
c.long<- lm(C~LONG_DD, data=temp.D2quant.mat3) #adj. R2 = 0.03879, p=0.004

d.lat<- lm(D~LAT_DD, data=temp.D2quant.mat3) #adj. R2 = -0.004724, p=0.7186
d.long<- lm(D~LONG_DD, data=temp.D2quant.mat3) #adj. R2 = 0.07301, p=0.000113

##Maps
#B map (using base map from above)
us.map.tempB<- us.map + geom_point(data=temp.D2quant.mat3, aes(x=LONG_DD, y=LAT_DD, size=B, colour=WSA_ECOREGION))
us.map.tempB<- us.map.tempB + scale_size(name="Species loss from T1 to T2") 
us.map.tempB<- us.map.tempB + ggtitle("Species loss component")

#C map
us.map.tempC<- us.map + geom_point(data=temp.D2quant.mat3, aes(x=LONG_DD, y=LAT_DD, size=C, colour=WSA_ECOREGION))
us.map.tempC<- us.map.tempC + scale_size(name="Species gain in T2") 
us.map.tempC<- us.map.tempC + ggtitle("Species gain component")

#D map
us.map.tempD<- us.map + geom_point(data=temp.D2quant.mat3, aes(x=LONG_DD, y=LAT_DD, size=D, colour=WSA_ECOREGION))
us.map.tempD<- us.map.tempD + scale_size(name="Dissimilarity (Beta-Div)") 
us.map.tempD<- us.map.tempD + ggtitle("Temporal Beta Diversity")
...
##Logistic regression of B and C components as predictors, and sig. D value as outcome.
#Species loss component (B)- P-value
temporalB.logr <- glm(p.dist.binary ~ Species_loss_component, data=temporalBD.BCD, family=binomial(link="logit"))
temporalB.logr
summary(temporalB.logr)

temporalBlogr.plot<- ggplot(temporalBD.BCD, aes(x=Species_loss_component, y=p.dist.binary)) + geom_point() + stat_smooth(method="glm", family="binomial", se=FALSE, size=1)
...
#Species gain component (C)- P-value
temporalC.logr <- glm(p.dist.binary ~ Species_gain_component, data=temporalBD.BCD, family=binomial(link="logit"))
temporalC.logr
summary(temporalC.logr)

temporalClogr.plot<- ggplot(temporalBD.BCD, aes(x=Species_gain_component, y=p.dist.binary)) + geom_point() + stat_smooth(method="glm", family="binomial", se=FALSE, size=1)
...
fig4c.plot<- ggplot(temporalBD.BCD.gainmean, aes(x=mean_Totalbeta, y=mean_Speciesgain, colour=WSA_ECOREGION)) + geom_point(size=4)
fig4c.plot<- fig4c.plot + labs(x="Mean temporal BD", y="Mean species gain") 
fig4c.plot<- fig4c.plot + theme_bw()
fig4c.plot<- fig4c.plot + theme(axis.text.x = element_text(colour="black", size=16))
fig4c.plot<- fig4c.plot + theme(axis.text.y = element_text(colour="black", size=16))
fig4c.plot<- fig4c.plot + theme(axis.title.x = element_text(size = rel(2), angle=00))
fig4c.plot<- fig4c.plot + theme(axis.title.y = element_text(size = rel(2), angle=90))
fig4c.plot<- fig4c.plot + annotate("text", x=0.27, y=0.35, label="(c)", size=10)
fig4c.plot<- fig4c.plot + theme(legend.title=element_text(size=16))
fig4c.plot<- fig4c.plot + theme(legend.text=element_text(size=16))
fig4c.plot<- fig4c.plot + scale_colour_brewer(type="qual", palette="Dark2", "Ecoregion (WSA)", breaks=c("CPL", "NAP", "NPL", "SPL", "TPL", "UMW", "WMT", "XER"), labels=c("Coastal Plains", "N. Appalachians", "N. Plains", "S. Plains", "Temperate Plains", "Upper MidWest", "W. Mountains", "Xeric"))  
...
#Means by ecoregion
fig4.dat.means<- as.data.frame(ddply(fig4.dat, "WSA_ECOREGION", summarize, 
                                           mean_Totalbeta = mean(Total_beta, na.rm=T), 
                                           mean_Speciescomponent = mean(Species_component, na.rm=T),
                                      

fig4c.plot<- ggplot(fig4.dat, aes(x=Total_beta, y=Species_component, colour=WSA_ECOREGION)) + geom_point()
...
##################################################################################################
####BETA-DIVERISTY AND LANDUSE/WATER QUALITY                                                     #
##################################################################################################

##Looking at spatial LCBD values more closely. 
#Use
surf.LCBD.summary #actual LCBD values are very small, can multiply by total BD so that #s are larger, possibly easier to work with. 
hist.LCBD.summary #as above. 
waterqual.dat.red 

#http://www.statmethods.net/advstats/cart.html

####Use a REGRESSION TREE approach to predict LCBD value for a lake from lake origin, ecoregion
#longitude, latitude and water quality variables. 
...
#1- lake origin, ecoregion, longitude, latitude
#Grow tree and display results #ADD ORIGIN 
surf.fit1<- rpart(LCBD~ORIGIN + WSA_ECOREGION + LONG_DD + LAT_DD, method="anova", data=surf.LCBD.env) 

printcp(surf.fit1) #Variables actually used in construction: lat, long, ecoregion 
plot(surf.fit1) #but no labels? 
summary(surf.fit1)

#Additional plots
par(mfrow=c(1,2))
rsq.rpart(surf.fit1) #don't really know how to interpet this. 

#Plot tree
plot(surf.fit1, uniform=TRUE, 
     main="Regression Tree for Surface LCBD")
text(surf.fit1, use.n=TRUE, all=TRUE, cex=.8)
...
# You can also choose cut-off manually (cp) (see example here from Zo)
tree_d2.pruned = prune(tree_d,cp=.044)   
tree_d2.pruned 
summary(tree_d2.pruned )
plot(as.party(tree_d2.pruned), tp_args = list(id = FALSE))
1-tree_d2.pruned $cptable[4,3]

##
plot(surf.fit2, uniform=TRUE,
     main="Regression Tree for Surface LCBD-water qual variables")
text(surf.fit2, use.n=TRUE, all=TRUE, cex=.8)
##

#3 land use (main variables)
...
plot(as.party(surf.fit3prune), tp_args = list(id = FALSE)) #issue? 
...
#R2 is 1-Rel Error
# R2      
1-surf.fit3$cptable[9,3]   #column 3, row 7 in cptable --> this gives you the adjusted R2 of the model. 
...
##Can use the code that Zo sent (MVpart) to get variance explained by each indidual node######
...
##Historical 
hist.LCBD.env<- as.data.frame(cbind(hist.LCBD.summary, nla.highC.hist$ORIGIN)) #don't add env since historical. 
colnames(hist.LCBD.env)[7]<- 'ORIGIN'  

#1- lake origin, ecoregion, longitude, latitude
hist.fit1<- rpart(LCBD~ORIGIN + WSA_ECOREGION + LONG_DD + LAT_DD, method="anova", data=surf.LCBD.env)

printcp(hist.fit1) #Variables actually used in construction: lat, long, ecoregion 
plot(hist.fit1) 
summary(hist.fit1)

plot(hist.fit1, uniform=TRUE, 
     main="Regression Tree for Historical LCBD")
text(hist.fit1, use.n=TRUE, all=TRUE, cex=.8)

##Logistic regression, sig D value as outcome, predictor- 2007 development extent (land use data)
#Percent development in basin (adjusted p-value). 
temporalDdevelopment.logr <- glm(p.adj.binary ~ PCT_DEVELOPED_BSN, data=temporalBD.BCD, family=binomial(link="logit"))
temporalDdevelopment.logr
summary(temporalDdevelopment.logr)

temporalDdevlogr.plot<- ggplot(temporalBD.BCD, aes(x=PCT_DEVELOPED_BSN, y=p.adj.binary)) + geom_point() + stat_smooth(method="glm", family="binomial", se=FALSE, size=1)

#Percent agriculture in basin (adjusted p-value). 
temporalDagric.logr <- glm(p.adj.binary ~ PCT_AGRIC_BSN, data=temporalBD.BCD, family=binomial(link="logit"))
temporalDagric.logr
summary(temporalDagric.logr)

temporalDaglogr.plot<- ggplot(temporalBD.BCD, aes(x=PCT_AGRIC_BSN, y=p.adj.binary)) + geom_point() + stat_smooth(method="glm", family="binomial", se=FALSE, size=1)

##Regression tree approach to partition D values by land use variables. 
#Create a matrix with D component as well as full set of land use variables (not specific forest types, overall land use). 
#Exclude open H20, water and ice/snow. 
...
plot(temporalland.fit1) 
summary(temporalland.fit1)

#Plot tree
par(mfrow=c(1,1))
plot(temporalland.fit1, uniform=TRUE, 
     main="Regression Tree for Temporal D and land use")
text(temporalland.fit1, use.n=TRUE, all=TRUE, cex=.8)
...
##Presence-absence data- using permutation method 1. 
