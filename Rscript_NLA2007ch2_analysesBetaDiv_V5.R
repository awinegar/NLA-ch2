...
#Previous script: Rscript_NLA2007ch2_analysesBetaDiv_V4.R
#R version: 3.1.2 (Pumpkin Helmet)

##Last update: November 2, 2015 
##Associated workspace: workspace_NLA2007ch2_analysesBetaDiv_V5.RData 
##Associated markdown: #ADD IN A MARKDOWN FILE WITH DESCRIPTIONS OF EACH SECTION. 
##Associated .txt of R script: Rscript_NLA2007ch2_analysesBetaDiv_V5.txt
...
#(URT figures etc. can stay- just number adjustments). --> This was done. 
...
dist.data<- read.csv("nla2007_spatialdistances_April2015.csv") #nla2007_spatialdistances_April2015.csv 
#Additional path: C:\Users\Winegardner\Documents\MCGILL\PhD chapters and projects\EPA National lakes Assessment\Chapter 2\NLA data\Env and spatial distance
...
surf.check<- (nla.diat.surf$SITE_ID %in% nla.highC.surf$SITE_ID) #All match- now n=176 for surface diatoms. 
...
##################################################################################################
####RADIOMETRIC DATING LOGISTIC REGRESSION AND CHI-SQAURE TESTS- CHECKED/OK                      #
##################################################################################################
...
ctbl<- table(dates.dat$Core_height_description, dates.dat$Pre_1850_binary) #contingency table 
...
surf.diat.abundnon<- as.data.frame(cbind(nla.diat.surf[,1], nla.diat.surf[,1216], nla.diat.surf[,1217], nla.diat.surf[,1218], surf.diat.abund[,4:927]))
...
surf.diat.abundh<- decostand(surf.diat.abund[,4:927], method="hell") #Hellinger on species columns. 
#Bind the Hellinger transformed species columns to site, ecoregion and lat/long columns. 
surf.diat.abundh<- as.data.frame(cbind(nla.diat.surf[,1], nla.diat.surf[,1216], nla.diat.surf[,1217], nla.diat.surf[,1218], surf.diat.abundh))
...
#Non-transformed
#Bind the non-transformed species columns to site, ecoregion and lat/long columns. 
hist.diat.abundnon<- as.data.frame(cbind(nla.diat.hist[,1], nla.diat.hist[,1216], nla.diat.hist[,1217], nla.diat.hist[,1218], hist.diat.abund[,3:1004]))
...
#Hellinger-transformed 
hist.diat.abundh<- decostand(hist.diat.abund[,3:1004], method="hell") #Hellinger on species columns. 
#Bind the Hellinger transformed species columns to site, ecoregion and lat/long columns. 
hist.diat.abundh<- as.data.frame(cbind(nla.diat.hist[,1], nla.diat.hist[,1216], nla.diat.hist[,1217], nla.diat.hist[,1218], hist.diat.abundh))
...
##Surface - beta.div
surf.beta.div <- beta.div(surf.diat.abundnon[,5:928], method="hellinger", sqrt.D=FALSE, samp=TRUE, nperm=999, save.D=FALSE, clock=FALSE)
#Hellinger transformation for calculating total BD. 
summary(surf.beta.div)
surf.beta.div$SStotal_BDtotal #SStotal = 132.04, BD total = 0.75
surf.beta.div$SCBD
surf.beta.div$LCBD
surf.beta.div$p.LCBD

#Without Hellinger transformation
surf.beta.div2 <- beta.div(surf.diat.abundnon[,5:928], sqrt.D=FALSE, samp=TRUE, nperm=999, save.D=FALSE, clock=FALSE)
summary(surf.beta.div2)
surf.beta.div2$SStotal_BDtotal #SStotal = 132.04, BD total = 0.75
surf.beta.div2$SCBD
surf.beta.div2$LCBD
...
surf.beta.div3 <- beta.div(surf.diat.abundnon[,5:928], method="percentagedifference", sqrt.D=FALSE, samp=TRUE, nperm=999, save.D=FALSE, clock=FALSE)
summary(surf.beta.div3)
surf.beta.div3$SStotal_BDtotal #SS total = 65.66, BD total = 0.375
...
##Historical - beta.div
hist.beta.div <- beta.div(hist.diat.abundnon[,5:1004], method="hellinger", sqrt.D=FALSE, samp=TRUE, nperm=999, save.D=FALSE, clock=FALSE)
summary(hist.beta.div)
hist.beta.div$SStotal_BDtotal #SStotal = 133.8, BD total = 0.764
hist.beta.div$SCBD
hist.beta.div$LCBD
hist.beta.div$p.LCBD

#Without Hellinger transformation
hist.beta.div2 <- beta.div(hist.diat.abundnon[,5:1004], sqrt.D=FALSE, samp=TRUE, nperm=999, save.D=FALSE, clock=FALSE)
summary(hist.beta.div2)
hist.beta.div2$SStotal_BDtotal #SS total = 133.76, BC total = 0.764
hist.beta.div2$SCBD
hist.beta.div2$LCBD
...
hist.beta.div3 <- beta.div(hist.diat.abundnon[,5:1004], method="percentagedifference", sqrt.D=FALSE, samp=TRUE, nperm=999, save.D=FALSE, clock=FALSE)
summary(hist.beta.div3)
hist.beta.div3$SStotal_BDtotal #SS total = 67.06, BD total = 0.383
...
#Chi-square test for cross between historical and surface sediments and sig/not sig. 
lcbdtbl<- read.csv("lcbd_contingency.csv") #lcbd_contingency.csv

clcbd<- table(lcbdtbl)

chisq.test(clcbd) #p < 0.05 --> significant, reject null, processes that produced the LCBD at the 2 time points not independent
#but related. 

#Correlation between the 2 vectors of LCBD in historical and surface sediments disregarding significance)
#Correlation between hist.LCBD.summary$LCBD and surf.LCBD.summary$LCBD 

lcbd.lm<- lm(surf.LCBD.summaryLCBD ~ hist.LCBD.summary$LCBD)
summary(lcbd.lm) #Adj R2 = 0.46 (p << 0.05) 
#also check with corr() 
...
us.map.ecoregion<- us.map.ecoregion + scale_colour_brewer(type="qual", palette="Dark2", "Ecoregion (WSA)", breaks=c("CPL", "NAP", "NPL", "SPL", "TPL", "UMW", "WMT", "XER"), labels=c("CPL", "NAP", "NPL", "SPL", "TPL", "UMW", "WMT", "XER"))  
# n = would have to change based on new n = 179. 
...
us.map.ecoregion<- us.map.ecoregion + theme(legend.position="bottom")
#export 
...
##Surface diatoms
surf.abund.bdc<- beta.div.comp(surf.diat.abundnon[,5:928], coef="S", quant=TRUE, save.abc=FALSE)
...
surf.abund.bdcCPL<- beta.div.comp(surf.diat.abundnonCPL[,5:928], coef="S", quant=TRUE, save.abc=FALSE)
surf.abund.bdcCPL$part

#NAP
surf.diat.abundnonNAP<- as.data.frame(subset(surf.diat.abundnon, WSA_ECOREGION == "NAP", drop=T))
surf.abund.bdcNAP<- beta.div.comp(surf.diat.abundnonNAP[,5:928], coef="S", quant=TRUE, save.abc=FALSE)
surf.abund.bdcNAP$part

#SPL
surf.diat.abundnonSPL<- as.data.frame(subset(surf.diat.abundnon, WSA_ECOREGION == "SPL", drop=T))
surf.abund.bdcSPL<- beta.div.comp(surf.diat.abundnonSPL[,5:928], coef="S", quant=TRUE, save.abc=FALSE)
surf.abund.bdcSPL$part

#TPL
surf.diat.abundnonTPL<- as.data.frame(subset(surf.diat.abundnon, WSA_ECOREGION == "TPL", drop=T))
surf.abund.bdcTPL<- beta.div.comp(surf.diat.abundnonTPL[,5:928], coef="S", quant=TRUE, save.abc=FALSE)
surf.abund.bdcTPL$part

#UMW
surf.diat.abundnonUMW<- as.data.frame(subset(surf.diat.abundnon, WSA_ECOREGION == "UMW", drop=T))
surf.abund.bdcUMW<- beta.div.comp(surf.diat.abundnonUMW[,5:928], coef="S", quant=TRUE, save.abc=FALSE)
surf.abund.bdcUMW$part

#WMT
surf.diat.abundnonWMT<- as.data.frame(subset(surf.diat.abundnon, WSA_ECOREGION == "WMT", drop=T))
surf.abund.bdcWMT<- beta.div.comp(surf.diat.abundnonWMT[,5:928], coef="S", quant=TRUE, save.abc=FALSE)
surf.abund.bdcWMT$part

##Historical diatoms 
hist.abund.bdc<- beta.div.comp(hist.diat.abundnon[,5:1004], coef="S", quant=TRUE, save.abc=FALSE)
...
hist.abund.bdcCPL<- beta.div.comp(hist.diat.abundnonCPL[,5:1004], coef="S", quant=TRUE, save.abc=FALSE)
hist.abund.bdcCPL$part

#NAP
hist.diat.abundnonNAP<- as.data.frame(subset(hist.diat.abundnon, WSA_ECOREGION == "NAP", drop=T))
hist.abund.bdcNAP<- beta.div.comp(hist.diat.abundnonNAP[,5:1004], coef="S", quant=TRUE, save.abc=FALSE)
hist.abund.bdcNAP$part

#SPL
hist.diat.abundnonSPL<- as.data.frame(subset(hist.diat.abundnon, WSA_ECOREGION == "SPL", drop=T))
hist.abund.bdcSPL<- beta.div.comp(hist.diat.abundnonSPL[,5:1004], coef="S", quant=TRUE, save.abc=FALSE)
hist.abund.bdcSPL$part

#TPL
hist.diat.abundnonTPL<- as.data.frame(subset(hist.diat.abundnon, WSA_ECOREGION == "TPL", drop=T))
hist.abund.bdcTPL<- beta.div.comp(hist.diat.abundnonTPL[,5:1004], coef="S", quant=TRUE, save.abc=FALSE)
hist.abund.bdcTPL$part

#UMW
hist.diat.abundnonUMW<- as.data.frame(subset(hist.diat.abundnon, WSA_ECOREGION == "UMW", drop=T))
hist.abund.bdcUMW<- beta.div.comp(hist.diat.abundnonUMW[,5:1004], coef="S", quant=TRUE, save.abc=FALSE)
hist.abund.bdcUMW$part

#WMT
hist.diat.abundnonWMT<- as.data.frame(subset(hist.diat.abundnon, WSA_ECOREGION == "WMT", drop=T))
hist.abund.bdcWMT<- beta.div.comp(hist.diat.abundnonWMT[,5:1004], coef="S", quant=TRUE, save.abc=FALSE)
hist.abund.bdcWMT$part


##Scatterplots of the explanatory components

all.bdc<- read.csv("total_bd_partitioning.csv") #total_bd_partitioning.csv 
#Additional path: C:\Users\Winegardner\Documents\MCGILL\PhD chapters and projects\EPA National lakes Assessment\Chapter 2
...
##total_bd_partitioning.csv spreadsheet would need to be updated with these results.##
...
colnames(nla.diat.new)[1215]<- 'MY_SAMPLE_ID'
colnames(nla.diat.new)[1216]<- 'WSA_ECOREGION'
colnames(nla.diat.new)[1217]<- 'LONG_DD'
colnames(nla.diat.new)[1218]<- 'LAT_DD'
...
temp.D2.pa<- decompose.D2(nla.diat.new.hist[,3:1214], nla.diat.new.surf[,3:1214], den.type=1)
...
surf.abund.fortempBD<- as.data.frame(subset(combo.wide, TYPE == "Surface", drop=T)) #6:1217 are columns
#Make rownames the SITE_ID
rownames(surf.abund.fortempBD)<- as.character(surf.abund.fortempBD[,1])

hist.abund.fortempBD<- as.data.frame(subset(combo.wide, TYPE == "Historical", drop=T)) #6:1217 are columns
rownames(hist.abund.fortempBD)<- as.character(hist.abund.fortempBD[,1])

#decompose.D2
temp.D2.quant<- decompose.D2(hist.abund.fortempBD[,6:1217], surf.abund.fortempBD[,6:1217], den.type=2)
...
except.sites2.BCD<- paired.diff2(hist.abund.fortempBD[,6:1217], surf.abund.fortempBD[,6:1217], method="%difference", pa.tr=FALSE, nperm=9999, permute.sp=1, BCD=TRUE, replace=FALSE, clock=FALSE)
...
summary(except.sites2.BCD$p.dist < 0.05) #41 sig #37 when run again
summary(except.sites2.BCD$p.adj < 0.05) #adjusted for multiple testing, 12 sig, 15 when run again
...
temporalBD.BCD<- as.data.frame(cbind(surf.diat.abundnon.label$SITE_ID, surf.diat.abundnon.label$WSA_ECOREGION, surf.diat.abundnon.label$LONG_DD, surf.diat.abundnon.label$LAT_DD, nla.highC.surf$ORIGIN, nla.highC.surf$LAKE_TYPE, nla.highC.surf$PCT_DEVELOPED_BSN, nla.highC.surf$PCT_AGRIC_BSN, nla.highC.surf$LAKE_TYPE_RECLASSED, temporalBD.BCD))
...
colnames(temporalBD.BCD)[9]<- 'LAKE_TYPE_RECLASSED'
...
write.csv(A.formosa, "A.formosa.csv")
write.csv(A.ambigua, "A.ambigua.csv")
...
##Diversity analyses steps

#Take wide-format surface and historical diatom dataframes with quantitative data and bind in ecoregion column
#surf.diat.abund   #quantitative data for surface diatoms (~927 species) #Use for now
#hist.diat.abund  #quantitative data for historical diatoms (~1004 species) #Use for now 
...
#All
hist.Srar <- rarefy(hist.eco[,3:1004], min(rowSums(hist.eco[,3:1004]))) 
hist.Srar #String
...
#All
surf.Srar <- rarefy(surf.eco[,4:927], min(rowSums(surf.eco[,4:927]))) #extra column in front of Site_ID
surf.Srar #String
...
#NAP
surf.NAP.Srar <- rarefy(surf.eco.NAP[,4:927], min(rowSums(surf.eco.NAP[,4:927]))) #extra column in front of Site_ID
surf.NAP.Srar #String
surf.NAP.Srar<- as.data.frame(surf.NAP.Srar)
summary(surf.NAP.Srar) #can get mean 

#SPL
surf.SPL.Srar <- rarefy(surf.eco.SPL[,4:927], min(rowSums(surf.eco.SPL[,4:927]))) #extra column in front of Site_ID
surf.SPL.Srar #String
surf.SPL.Srar<- as.data.frame(surf.SPL.Srar)
summary(surf.SPL.Srar) #can get mean 

#TPL
surf.TPL.Srar <- rarefy(surf.eco.TPL[,4:927], min(rowSums(surf.eco.TPL[,4:928]))) #extra column in front of Site_ID
surf.TPL.Srar #String
surf.TPL.Srar<- as.data.frame(surf.TPL.Srar)
summary(surf.TPL.Srar) #can get mean 

#UMW
surf.UMW.Srar <- rarefy(surf.eco.UMW[,4:927], min(rowSums(surf.eco.UMW[,4:927]))) #extra column in front of Site_ID
surf.UMW.Srar #String
surf.UMW.Srar<- as.data.frame(surf.UMW.Srar)
summary(surf.UMW.Srar) #can get mean 

#WMT
surf.WMT.Srar <- rarefy(surf.eco.WMT[,4:927], min(rowSums(surf.eco.WMT[,4:927]))) #extra column in front of Site_ID
...
#All
hist.Shannon<- diversity(hist.eco[,3:1004], index = "shannon")
summary(hist.Shannon)

#CPL
hist.CPL.Shannon<- diversity(hist.eco.CPL[,3:1004], index = "shannon")
summary(hist.CPL.Shannon)

#NAP
hist.NAP.Shannon<- diversity(hist.eco.NAP[,3:1004], index = "shannon")
summary(hist.NAP.Shannon)

#SPL
hist.SPL.Shannon<- diversity(hist.eco.SPL[,3:1004], index = "shannon")
summary(hist.SPL.Shannon)

#TPL
hist.TPL.Shannon<- diversity(hist.eco.TPL[,3:1004], index = "shannon")
summary(hist.TPL.Shannon)

#UMW
hist.UMW.Shannon<- diversity(hist.eco.UMW[,3:1004], index = "shannon")
summary(hist.UMW.Shannon)

#WMT
hist.WMT.Shannon<- diversity(hist.eco.WMT[,3:1004], index = "shannon")
summary(hist.WMT.Shannon)

#Shannon alpha diversity for surface sediment subsets
#All
surf.Shannon<- diversity(surf.eco[,4:927], index = "shannon")
summary(surf.Shannon)

#CPL
surf.CPL.Shannon<- diversity(surf.eco.CPL[,4:927], index = "shannon")
summary(surf.CPL.Shannon)

#NAP
surf.NAP.Shannon<- diversity(surf.eco.NAP[,4:927], index = "shannon")
summary(surf.NAP.Shannon)

#SPL
surf.SPL.Shannon<- diversity(surf.eco.SPL[,4:927], index = "shannon")
summary(surf.SPL.Shannon)

#TPL
surf.TPL.Shannon<- diversity(surf.eco.TPL[,4:927], index = "shannon")
summary(surf.TPL.Shannon)

#UMW
surf.UMW.Shannon<- diversity(surf.eco.UMW[,4:927], index = "shannon")
summary(surf.UMW.Shannon)

#WMT
surf.WMT.Shannon<- diversity(surf.eco.WMT[,4:927], index = "shannon")
summary(surf.WMT.Shannon)


#Simpson alpha for historical sediment subsets 
#All
hist.Simpson<- diversity(hist.eco[,3:1004], index = "simpson")
summary(hist.Simpson)

#CPL
hist.CPL.Simpson<- diversity(hist.eco.CPL[,3:1004], index = "simpson")
summary(hist.CPL.Simpson)

#NAP
hist.NAP.Simpson<- diversity(hist.eco.NAP[,3:1004], index = "simpson")
summary(hist.NAP.Simpson)

#SPL
hist.SPL.Simpson<- diversity(hist.eco.SPL[,3:1004], index = "simpson")
summary(hist.SPL.Simpson)

#TPL
hist.TPL.Simpson<- diversity(hist.eco.TPL[,3:1004], index = "simpson")
summary(hist.TPL.Simpson)

#UMW
hist.UMW.Simpson<- diversity(hist.eco.UMW[,3:1004], index = "simpson")
summary(hist.UMW.Simpson)

#WMT
hist.WMT.Simpson<- diversity(hist.eco.WMT[,3:1004], index = "simpson")
summary(hist.WMT.Simpson)

#Simpson slpha for surface sediment subsets
#All
surf.Simpson<- diversity(surf.eco[,4:927], index = "simpson")
summary(surf.Simpson)

#CPL
surf.CPL.Simpson<- diversity(surf.eco.CPL[,4:927], index = "simpson")
summary(surf.CPL.Simpson)

#NAP
surf.NAP.Simpson<- diversity(surf.eco.NAP[,4:927], index = "simpson")
summary(surf.NAP.Simpson)

#SPL
surf.SPL.Simpson<- diversity(surf.eco.SPL[,4:927], index = "simpson")
summary(surf.SPL.Simpson)

#TPL
surf.TPL.Simpson<- diversity(surf.eco.TPL[,4:927], index = "simpson")
summary(surf.TPL.Simpson)

#UMW
surf.UMW.Simpson<- diversity(surf.eco.UMW[,4:927], index = "simpson")
summary(surf.UMW.Simpson)

#WMT
surf.WMT.Simpson<- diversity(surf.eco.WMT[,4:927], index = "simpson")
...
#Transform Shannon dataframe to exp(H). (converting to effective numbers)
surf.Eff.dframe<- as.data.frame(exp(surf.Shannon.dframe))
colnames(surf.Eff.dframe)[1]<- 'surf.Effective'
...
summary(surf.effenvprune)

par(mfrow=c(1,2)) 
rsq.rpart(surf.effenvprune)  # visualize cross-validation result
#R2 is 1-Rel Error
# R2      
1-surf.effenvprune$cptable[2,3]   #column 3, row 2 in cptable --> this gives you the adjusted R2 of the model. #correct
#R = 
plot(as.party(surf.effenvprune), tp_args = list(id = FALSE))

#Land cover 
surf.Eff.land<- as.data.frame(cbind(surf.Eff.dframe, nla.highC.surf[,25:42]))
surf.Eff.land2<-  rpart(surf.Effective~PCT_DEVELOPED_BSN + PCT_BARREN_BSN + PCT_FOREST_BSN + PCT_AGRIC_BSN + PCT_WETLAND_BSN, method="anova", data=surf.Shannon.land)
printcp(surf.Eff.land2) #Variables actually used in construction: 
plot(surf.Eff.land2)
summary(surf.Eff.land2)
surf.efflandprune<-  prune(surf.Eff.land2, cp=surf.Eff.land2$cptable[which.min(surf.Eff.land2$cptable[,"xerror"]),"CP"])
summary(surf.efflandprune)

par(mfrow=c(1,2)) 
rsq.rpart(surf.efflandprune)  # visualize cross-validation result
#R2 is 1-Rel Error
# R2      
1-surf.efflandprune$cptable[8,3]   #column 3, row 8 in cptable --> this gives you the adjusted R2 of the model. #correct
#R = 
plot(as.party(surf.efflandprune), tp_args = list(id = FALSE))
